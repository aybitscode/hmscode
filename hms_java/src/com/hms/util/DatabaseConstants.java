...
//	public static String TABLE_EMPLOYEES = "select firstName, lastName, username, password, email, mobile, birthDate, gender from employees where role = '"+Constants.USER+"'";
	...
	//public static String TABLE_MOBILE = "select mobile from employees where role = '"+Constants.USER+"'";
	...
	public static String BOOKING_ID_USER = "select bookingID from booking where invoiceID is null or invoiceID not like '"+Constants.PREFIX_TAX_LESS+"%'";
	...
	public static String TABLE_BOOKING_STATUS_USER = "select bookingID, firstName, lastName, b1.mobile, bookingDate, bookedDate, checkoutDate, bookingTotalNights, grossAmount, status, paymentMode from booking b1 inner join customers c1 on b1.mobile = c1.mobile where status = ? and invoiceID not like '"+Constants.PREFIX_TAX_LESS+"%' order by bookingID desc";
	public static String TABLE_BOOKING_MODE_CASH_TAX = "select bookingID, firstName, lastName, b1.mobile, bookingDate, bookedDate, checkoutDate, bookingTotalNights, grossAmount, status, paymentMode from booking b1 inner join customers c1 on b1.mobile = c1.mobile where paymentMode = ? and invoiceID like '"+Constants.PREFIX_CASH+"%' order by bookingID desc";
	public static String TABLE_BOOKING_MODE_TAX_LESS = "select bookingID, firstName, lastName, b1.mobile, bookingDate, bookedDate, checkoutDate, bookingTotalNights, grossAmount, status, paymentMode from booking b1 inner join customers c1 on b1.mobile = c1.mobile where paymentMode = ? and invoiceID like '"+Constants.PREFIX_TAX_LESS+"%' order by bookingID desc";
	...
	public static String UPDATE_BOOKING = "update booking set " +
			...
			"invoiceID = ?"+
			"where bookingID = ?";
	public static String SELECT_CHECKIN_BOOKING_ID = "select bookingID from booking where status = '"+Constants.CHECKIN+"'";
	...
	public static String BOOKING_CHECKOUT = "select t1.bookingID, bookedDate, t2.checkinDate, checkoutDate, roomCategoryID, roomDoorNumber, mobile, bookingRoomCost, facilitiesCost, bookingTotalNights,advanceAmt from booking t1 inner join checkin t2 on t1.bookingID = t2.bookingID where t1.bookingID = ? and roomDoorNumber = ?";
	...
	public static String CURRENT_BOOKED_DATE = "select bookingID, bookedDate, roomCategoryID, roomDoorNumber, mobile from booking where bookedDate between DATE_ADD(current_date() , INTERVAL '00:00:01' HOUR_SECOND) and DATE_ADD(current_date() , INTERVAL '23:59:59' HOUR_SECOND) and status ='"+Constants.BOOKED+"'";
	public static String CURRENT_CHECKOUT_DATE = "select bookingID, checkoutDate, roomCategoryID, roomDoorNumber, mobile from booking where checkoutDate between DATE_ADD(current_date() , INTERVAL '00:00:01' HOUR_SECOND) and DATE_ADD(current_date() , INTERVAL '23:59:59' HOUR_SECOND) and status = '"+Constants.CHECKIN+"'";
	public static String CURRENT_CANCEL = "select bookingID, checkoutDate, roomCategoryID, roomDoorNumber, mobile from booking where checkoutDate <= current_timestamp() and status = '"+Constants.BOOKED+"'";
	...
	//public static final String MULTIPLE_FREE_ROOMS = "select roomDoorNumber, bookingID from booking where status = '"+Constants.BOOKED+"'"+" and bookingID not in ( select bookingID from booking where bookedDate < ? and checkoutDate < ? and status = '"+Constants.BOOKED+"'"+" or bookedDate > ? and checkoutDate > ? and status = '"+Constants.BOOKED+"')";
	public static final String SELECTED_DATE_ROOMS_BOOKED = "select roomDoorNumber, bookingID from booking where status = '"+Constants.BOOKED+"'"+" and bookingID not in ( select bookingID from booking where bookedDate < ? and checkoutDate < ? and status = '"+Constants.BOOKED+"'"+" or bookedDate > ? and checkoutDate > ? and status = '"+Constants.BOOKED+"')";
	public static final String SELECTED_DATE_ROOMS_CHECKEDIN = "select roomDoorNumber, bookingID from booking where status = '"+Constants.CHECKIN+"'"+" and bookingID not in ( select bookingID from booking where bookedDate < ? and checkoutDate < ? and status = '"+Constants.CHECKIN+"'"+" or bookedDate > ? and checkoutDate > ? and status = '"+Constants.CHECKIN+"')";
	...
	public static String TABLE_CHECKOUT_COLS = "select b1.bookingID, roomDoorNumber, firstName, lastName, b1.mobile, c1.checkOutDate from booking b1 inner join customers ct on b1.mobile = ct.mobile inner join checkout c1 on b1.bookingID = c1.bookingID where status = '"+Constants.CHECKOUT+"' order by c1.checkOutDate desc";
	//public static String TABLE_CHECK_IN_COLS = "select b1.bookingID, roomDoorNumber, firstName, lastName, b1.mobile, checkInDate, totalAdults, totalChilds, advanceAmt from booking b1 inner join customers ct on b1.mobile = ct.mobile inner join checkin c1 on b1.bookingID = c1.bookingID where status = '"+Constants.CHECKIN+"'";
