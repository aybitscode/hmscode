...
public class ViewAllRooms extends JPanel {
	private JTable table;
	DefaultTableModel tableModel;
	JLabel transExcel;
	String filePath;
	private RoomController objCon;
	...
	MainPage mpg;
	public ViewAllRooms(final MainPage mpg) {
		this.mpg = mpg;
...
//				lblCustomers.setForeground(new Color(50, 197, 210));
				...
				ConfigurationPanel.lblRoomNumbers.requestFocus(true);
			}
		});
		...
		lblCustomerHistory = new JLabel("All Rooms");
		...
		objCon = new RoomController(tableModel, table);
		objCon.retrieveAll(DatabaseConstants.TABLE_ROOM_COLS);
...
//     			}
		...
		comboSearch.getEditor().getEditorComponent().addKeyListener(new KeyAdapter() {
		    ...
		    public void keyReleased(KeyEvent event) {
		        if (event.getKeyChar() == KeyEvent.VK_ENTER) {
		            ...
		            {		            	
		            	objCon.retrieve(DatabaseConstants.TABLE_ROOM_COLS_DNO, ""+comboSearch.getSelectedItem());
		            }
		...
		sbm_customer_mobile = new SearchBoxModel(comboSearch, DatabaseConstants.ROOM_DOOR_NUMBER);
