...
public class TaxApiValidator {
    ...
    protected void validate(TaxRule taxRule) throws HMSRuntimeException {
        ...
        if(HMSAPIValidator.isBlankString(taxRule.getTaxRuleId())){
            throw new HMSRuntimeException(HMSErrorInfo.getNewErrorInfo(HMSErrorCodes.INVALID_HOTEL_SETUP_DATA,"Invalid Tax ID provided or tax-id is null"));
        }

        if(taxRule.getTaxRuleId().length() > HMSAPIConstants.HMS_ID_LENGTH){
            throw new HMSRuntimeException(HMSErrorInfo.getNewErrorInfo(HMSErrorCodes.HMS_LENGTH_CHECK_FAILED," ID length is more than the allowed size"));
        }

     /**   if(null == taxRule.getHotelAttributes()){
      ...
      there is no taxrule attributes
        }**/

        if(HMSAPIValidator.isBlankString(taxRule.getHotelAttributes().getHotelName())){
        ...
        hmsapiValidator.validateContactDetails(taxRule.getHotelAttributes().getHotelContactDetails());
        hmsapiValidator.validateAddress(taxRule.getHotelAttributes().getHotelAddress());
