...
CREATE TABLE `hms_hotel_registration_data` (
  ...
  `date_updated`timestamp NULL DEFAULT NULL,
  `date_deleted`timestamp NULL DEFAULT NULL
)COMMENT='This table holds all the legal details of the Identities related to the hotel' ;

CREATE TABLE `hms_room_category` (
  `hotel_id` varchar(60) COMMENT 'Unique identifier to identify a given hotel',
  `room_category_id` varchar(60) ,
...
CREATE TABLE `hms_employee` (
  ...
  `hotel_id` varchar(60) COMMENT 'This is the foreign key reference to the hotel_id created in the hms_hotel table.',
  `date_created`timestamp,
  `date_updated`timestamp NULL DEFAULT NULL,
  `date_deleted`timestamp NULL DEFAULT NULL
...
CREATE TABLE `hms_customer` (
  ...
  `hotel_id` varchar(60) COMMENT 'This is the foreign key reference to the hotel id created in the hms_hotel table',
...
CREATE TABLE `hms_identification_params` (
  `hotel_id` varchar(60) COMMENT 'Unique identifier to identify a given hotel',
  `IDENTIFICATION_PARAM_ID` varchar(60),
  `IDENTIFICATION_PARAM_TYPE` varchar(60),
  `IDENTIFICATION_PARAM_FILE` BLOB(2000),
  `date_created`timestamp,
  `date_updated`timestamp NULL DEFAULT NULL,
  `date_deleted`timestamp NULL DEFAULT NULL
) COMMENT='This table holds the information of all the customer data.';
...
CREATE TABLE `hms_voucher` (
  ...
  `date_updated`timestamp NULL DEFAULT NULL,
  `date_deleted`timestamp NULL DEFAULT NULL
) COMMENT='This table defines the vouchers that are created by a hotel and applied to any hotel operation';

CREATE TABLE `hms_facility` (
  `hotel_id` varchar(60) COMMENT 'Unique identifier to identify a given hotel',
  `facility_id` varchar(60) COMMENT 'The unique identifier to identify the facility assigned',
  `facility_name` varchar(45) COMMENT 'The key to define the facility to show during reporting etc.',
  `facility_description` varchar(100) ,
  `is_available` int(11) ,
  `is_chargeable` int(11) ,
  `facility_type` int(11),
  `facility_charges` double,
  `date_created`timestamp,
  `date_updated`timestamp NULL DEFAULT NULL,
  `date_deleted`timestamp NULL DEFAULT NULL
...
CREATE TABLE `hms_facility_voucher_map` (
  `hotel_id` varchar(60) COMMENT 'This column represents the unique hotel ID generated during hotel setup and uniquely identifies the hotel created in hms solution',
...
CREATE TABLE `hms_booking` (
  ...
  `booking_id` varchar(60) COMMENT 'The entire billing details are mapped to this id - bill_id which is a foreign key to the billing_info table ',
  `checkout_date` datetime COMMENT 'This refers to the check-in and check-out details and the no of rooms booked by an individual user.',
  `invoice_id` varchar(60) ,
  `booking_status` varchar(60) COMMENT 'The Booking status is - 0 - undetermined, 1 - confirmed,2 - cancelled.',
  `date_created`timestamp,
  `date_updated`timestamp NULL DEFAULT NULL,
  `date_deleted`timestamp NULL DEFAULT NULL
);
...
)COMMENT='This is an application specific table that records all the version information to record the DB,UX and App Versions for Reporting.';
